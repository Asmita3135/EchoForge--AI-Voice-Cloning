# EchoForge — Member 4 Final Antigravity Implementation Specification

**Document status:** Final. Based on a direct source-code audit of the real
Member 1, Member 2, and Member 3 repositories — not on assumptions, not on
...
API). Members 1–3 are complete and must not be redesigned. Follow the
...
real source code audited in Section B — do not substitute a different
...
1. Inspect before editing — re-open the actual file before changing it, even if this document already describes it.
2. Never assume an interface — if a function's real signature differs from what's written here, trust the code and flag the discrepancy.
3. Never rewrite a completed module unnecessarily.
4. Preserve Member 1–3 behavior exactly except where Section D explicitly authorizes a change.
5. Make small, reviewable changes — one milestone, one focused diff.
...
9. Stop and ask/flag when actual code differs from this spec's assumptions — do not silently paper over it.
10. Never silently modify model behavior (scoring, thresholds, class labels).
11. Never silently change an existing Member 1/2/3 threshold — if a threshold looks wrong (see Section D), surface it, don't quietly "fix" it inside Member 4.
12. Never claim success without actually running the relevant test.
13. Keep Member 4 modular — normalization, reliability, risk, decision, and explainability are separate files, each independently testable.
14. Keep the core pipeline runnable and testable without FastAPI.
15. Make uncertainty explicit in the data — a missing module output is `None`, never `0`.
16. Prefer `INCONCLUSIVE` over unjustified `LOW`/`HIGH`.
17. Keep all thresholds/weights/margins in one config file — no magic numbers inline.
18. Avoid magic numbers — every number in code should trace to a named constant with a comment explaining where it comes from.
...
> that evidence, is this audio **LOW**, **HIGH**, or **INCONCLUSIVE** risk —
> and why?

Member 4's five jobs, in the order they execute:

1. **Integration** — call Members 1–3 through small adapters, without modifying their internals beyond one minimal, explicitly-scoped change to Member 2 (Section D).
2. **Normalization** — convert three differently-scaled, differently-directioned scores into one consistent `0 = low risk, 100 = high risk` convention.
3. **Reliability** — separately score how much to trust the evidence (audio quality, module confidence, missing/degraded modules) — this is not the same number as risk.
4. **Risk aggregation + decision** — combine normalized risk into one score, then apply a decision engine that defaults to `INCONCLUSIVE` whenever risk is high but reliability is low, or the evidence is too close to a boundary.
5. **Explainability + API** — return a JSON response, over a FastAPI `POST /analyze` endpoint, whose `reasons` are always derived from real evidence fields, never hardcoded strings.

---

## B. Current Architecture (audit results — ground truth)

### B.1 Member 1 — Deepfake Voice Detection — ✅ COMPLETE, cleanly importable
...
- **Model:** `Gustking/wav2vec2-large-xlsr-deepfake-audio-classification` (HuggingFace `AutoModelForAudioClassification`), loaded once via `model.detector.get_detector()` (singleton pattern already implemented — Member 4 does not need to add its own caching layer for this).
...
- **Score direction:** `raw_score` = P(fake), 0–1, **higher = more synthetic**. Already risk-aligned; no inversion needed.
- **Failure mode:** does **not** raise on missing/corrupt file — returns a valid dict with `classification: "UNCERTAIN"`, `confidence: "LOW"`, plus a top-level `"error"` key. **Member 4's adapter must check for the presence of an `"error"` key**, not rely on a try/except to detect this case.
...
### B.2 Member 2 — Speaker Verification — ✅ COMPLETE logic, ⚠️ CLI-only wrapper

- **As shipped:** `verify_speaker.compare_speakers(reference_input, test_input)` only **prints** a formatted report and calls `sys.exit(1)` on any error path. It has no `return` statement — callers get `None`. This is the single biggest integration gap in the project.
- **Model:** SpeechBrain `speechbrain/spkrec-ecapa-voxceleb` (ECAPA-TDNN), 192-dim speaker embeddings, cosine similarity.
- **Confirmed thresholds — found in the actual code, `verify_speaker.py`:**
  ```python
  ...
  #   else                -> "UNCERTAIN"
  ```
  **Discrepancy found:** Member 2's own `README.md` documents `PROTOTYPE_THRESHOLD = 0.5000` and different testing guidance numbers (≥0.55 "same", ≤0.45 "different"). The README is stale. **Member 4 must use the code's real values (0.60 ± 0.05), never the README's.** This should be reported back as a documentation bug in Member 2's repo, but Member 4's own config must not inherit the wrong number.
- **Score direction:** cosine similarity, roughly 0–1, **higher = more likely genuine/same speaker** — opposite of Member 4's risk convention, requires inversion (Section G).
- **Input shape is different from Members 1 and 3:** it needs **two** audio files — `reference_input` (claimed speaker) and `test_input` (audio under investigation) — not one. Member 4's API must treat reference audio as optional, and speaker verification as skippable, not failable, when it's absent.
...
- **No formal reliability signal** beyond a README note that clips under ~1.5s or noisy/reverberant audio reduce trustworthiness — Member 4 must derive speaker-channel reliability itself from duration/quality, it cannot read it from Member 2's output.

### B.3 Member 3 — STT + Context Analysis — ✅ COMPLETE logic, ⚠️ script-only, one real path bug

- **Transcription:** `transcribe.transcribe_audio(audio_path) -> (transcript: str, segments: list[dict])`. Uses OpenAI Whisper, model size hardcoded to `"tiny"`. **The Whisper model loads at import time** (`model = whisper.load_model("tiny")` runs the moment `transcribe.py` is imported) — this means the first import is slow (multi-second) and should happen once, at Member 4's process startup, never per-request.
- **Context analysis:** `context_analyzer.analyze_context(transcript) -> dict`:
  ...
  ```
  Score is 0–100, capped at 100, **higher = more suspicious** — already risk-aligned, no inversion needed. `risk_level` bands (from `context_analyzer.py`): `>=60` HIGH, `>=30` MEDIUM, else LOW.
- **Real bug, confirmed in Member 3's own README "Important Implementation Notes":** `keywords.json` defines `credential_request_phrases` and `urgency_phrases` categories, but `SCORES` and `REASONS` dicts in `context_analyzer.py` have no entries for either — matches in those two categories land in the returned `detected{}` dict but contribute **0 score and no reason text**. This is existing, documented Member 3 behavior. **Member 4 must not "fix" this** (out of Member 4's ownership per the team boundary rules) — but the explainability layer must be aware `detected` can contain categories that did not move the score, so it never misreports them as risk drivers.
- **Path bug Member 4 will hit immediately:** `context_analyzer.load_keywords()` does `open("keywords.json", "r")` — a bare relative path. This only works if the process's working directory happens to be Member 3's own folder. **This is the one place a small, safe fix is justified** (Section D) — pointing the `open()` call at an absolute path derived from `__file__`, changing zero behavior.
- **No callable end-to-end function** — `main.py` and `main_with_conv.py` are scripts with a hardcoded `audio_path` variable at the top and no function boundary; there is no `run(audio_path)` to import. Member 4 needs a thin wrapper (Section D/E).
- **No error handling at all** around Whisper or file access in Member 3's code — a bad file raises an uncaught exception. Member 4's adapter must catch this.
...
adapters/member1_adapter.py   adapters/member2_adapter.py     adapters/member3_adapter.py
  ...
  (unmodified import)            (after minimal return-value       + analyze_context()
                                   fix — Section D)                 (via new run() wrapper)
        │                               │                               │
        ▼                               ▼                               ▼
  ...
  {status, data, error}           {status, data, error}           {status, data, error}
        └───────────────────────────────┼───────────────────────────────┘
                        ...
                        (deepfake_risk, speaker_mismatch_risk, context_risk)
                                        │
                                        ▼
                              reliability.py  →  reliability_score, reliability_reasons[]
                                        │
                                        ▼
                              risk_engine.py  →  overall_risk_score
                                        │
                                        ▼
                              decision_engine.py  →  LOW | HIGH | INCONCLUSIVE, human_review_required
                                        │
                                        ▼
                              explainability.py  →  reasons[], warnings[]
                                        │
                                        ▼
                                  JSON response
                                        │
...
## D. Required Changes to Members 1–3 (minimal, explicit, nothing else)

### Change 1 — Member 2: make `compare_speakers` return data (REQUIRED)
...
process. **Every existing `print()` statement stays exactly as-is** — the
...
load-model → embed → cosine-similarity → threshold steps (using the exact
...
### Change 2 — Member 3: fix the relative `keywords.json` path (REQUIRED, trivial, zero behavior change)
...
This changes zero scoring behavior — it only makes the existing behavior
work when called from outside Member 3's own directory.

### Change 3 — Member 3: add a callable pipeline wrapper (REQUIRED, additive only)
...
this can instead live entirely inside Member 4's adapter — see Section
...
- `Member 1/model/detector.py`, `Member 1/inference/scoring.py`, `Member 1/inference/pipeline.py`, `Member 1/audio/diagnostics.py`, `Member 1/config.py` — Member 1 is fully usable as-is.
...
- `Member 3/context_analyzer.py`'s `SCORES`, `REASONS`, `get_risk_level()`, and `analyze_context()` scoring logic — including the known dead-category behavior described in Section B.3. That belongs to Member 3.
- `Member 3/keywords.json` content.
- Any `requirements.txt` belonging to Members 1–3 (Member 4 has its own).
...
├── .env.example
│
├── config.py                     # ALL thresholds/weights/margins live here
│
├── adapters/
│   ├── __init__.py
│   ├── result.py                 # shared AdapterResult type
│   ├── member1_adapter.py        # wraps analyze_audio()
│   ├── member2_adapter.py        # wraps compare_speakers() (post Change 1)
│   └── member3_adapter.py        # wraps run_context_pipeline() (post Change 3)
│
├── core/
│   ├── __init__.py
│   ├── normalization.py          # Section G
│   ├── reliability.py            # Section H
│   ├── risk_engine.py            # Section I
│   ├── decision_engine.py        # Section J
│   ├── explainability.py         # Section K
│   └── pipeline.py               # orchestrates all of the above, no FastAPI import here
│
├── api/
│   ├── __init__.py
│   ├── main.py                   # FastAPI app, startup model preload, /analyze, /health
│   ├── schemas.py                # Pydantic request/response models
│   └── file_handling.py          # temp file staging, validation, cleanup (Section L/security)
│
├── tests/
│   ├── unit/
...
│   ├── integration/
│   │   ├── test_adapters_mocked.py   # Members 1-3 mocked, no real models loaded
│   │   └── test_pipeline_mocked.py
│   ├── e2e/
│   │   └── test_pipeline_real.py     # uses real Member 1-3 modules + sample audio
│   └── api/
│       └── test_analyze_endpoint.py  # FastAPI TestClient
│
├── sample_audio/                 # small test clips reused from Members 1-3's own /audio folders
└── external/                     # NOT copied source — see note below
```

**Note on `external/`:** Member 4's code imports Members 1–3 directly from
their existing repo locations (adjusting `sys.path` or installing them as
local editable packages — whichever is simpler on Windows for a beginner;
Section P gives the exact commands). Do not copy-paste Member 1/2/3 source
files into Member 4's repository — that creates two diverging copies of
...
— it is an expected, non-alarming state, distinct from `"error"`, and the
...
no request body Pydantic model for `/analyze` itself — FastAPI's
...
## G. Normalization — exact transformations
...
|---|---|---|---|---|---|
| `deepfake_risk` | P(audio is AI-generated), Member 1 `raw_score` | 0.0–1.0, higher = more synthetic | `raw_score * 100` | 0–100, higher = more synthetic-risk | None needed — already risk-aligned. Directly reflects model confidence, not acoustic quality (quality is handled separately, in reliability). |
| `speaker_mismatch_risk` | Cosine similarity to reference speaker, Member 2 | ~0.0–1.0 in practice, higher = more likely same speaker | Piecewise-linear, anchored to Member 2's own real thresholds (0.65 / 0.55) — see code below | 0–100, higher = more likely different/mismatched speaker | Cosine similarity is not a calibrated probability; the piecewise mapping preserves Member 2's own decision boundaries but the in-between values are a linear approximation, not a learned calibration. |
| `context_risk` | Keyword/phrase-based suspicion score, Member 3 `context_score` | 0–100, capped, higher = more suspicious | Identity — no transform | 0–100, higher = more suspicious | Inherits Member 3's own documented limitation: substring keyword matching, prone to false positives (e.g., the word "bank" alone), and the two "phrases" categories that score 0 due to the Member 3 bug in Section B.3. |
...
normalized risk is `None`. **`None` must never be coerced to `0`** — a
...
Reliability is independent of risk direction — it answers "how much
should we trust this risk number," not "is it risky." 0–100 scale,
...
- If Member 2 was `"skipped"` (no reference provided), the ceiling for this section is capped at **25** instead of 40 — it's an expected gap, not a fault, but it still limits how much can be known.

**Audio quality — 35 points max** (sourced entirely from Member 1's `extended_diagnostics`, the only module with real acoustic metrics, since all three channels share the same input audio)
...
**Decision confidence — 25 points max**
- Member 1's own `confidence` field: `HIGH` → **+10**, `MODERATE` → **+5**, `LOW` → **+0**
- Penalty (not bonus) when a module's own uncertainty logic fired: **−10** per flagged channel (Member 1 landed inside its own `UNCERTAINTY_MARGIN`; Member 2 landed inside its own 0.55–0.65 band), capped at **−25** total. This penalty is separate from, and stacks with, the availability/quality points above — it specifically penalizes "the module ran fine but told us it wasn't sure," which availability/quality checks don't capture.

```python
# core/reliability.py — signature
...
specific module's real output — this must remain true; if a future
...
correctness — configurable and revisitable):
- `deepfake_risk` gets the most weight (0.5) because Member 1 is the only
  ...
  diagnostics — it's the most directly relevant to "is this audio itself
...
for `INCONCLUSIVE` — never as "risk score of zero."
...
only for `LOW` — matching the pipeline diagram's human-review step, which
...
signal — Member 2 and Member 3 being absent lowers reliability (Section
...
```python
# core/explainability.py — behavior, not exact code
...
unconditionally — every one is gated on a real field crossing a real,
...
| Audio shorter than `MIN_RELIABLE_DURATION_SEC` | Analysis uncertainty | Not a hard rejection — Member 1 itself already returns `classification: "UNCERTAIN"` for this; Member 4 passes it through and lets reliability/decision handle it |
...
| Member 3 returns malformed data (missing key) | Module failure | Adapter validates expected keys are present before returning `"ok"`; missing keys → `status: "error"` with a specific message, never a `KeyError` propagating upward |
| A score is `NaN` or `Inf` | Invalid module output | Adapter treats this the same as `status: "error"` for that channel — `math.isfinite()` check before accepting any numeric score from any module |
| Model loading failure at startup | Server fault | FastAPI startup fails loudly with a clear log message — do not start serving `/analyze` requests with a partially-loaded model set |
| Any other unexpected exception inside `/analyze` | Server fault | Caught by a single top-level handler in `api/main.py`, logged with the `request_id`, returns HTTP 500 with a generic message — **never** a raw Python traceback in the response body |
...
about a module misbehaving on valid input, it's evidence uncertainty —
...
```python
# core/pipeline.py — no FastAPI import anywhere in this file
def run_analysis(audio_path: str, reference_audio_path: Optional[str]) -> AnalysisResult:
    ...
    # at the API layer, or a simple ThreadPoolExecutor here) — they are
...
`uvicorn` being installed. That's what makes Milestones M4.9–M4.11
...
— it does not contain any decision logic itself.

```
POST /analyze
  Content-Type: multipart/form-data
  Fields:
    audio            : file, required   — the suspicious/test audio
    reference_audio  : file, optional   — claimed-speaker reference; enables speaker verification
  → 200 OK  → AnalyzeResponse (Section F.4), always returned when the
              request itself was valid, even if one or more modules failed
              internally (that's evidence uncertainty, not an API error)
  → 400 Bad Request → { "detail": "..." }  (invalid/missing file, bad format, too large)
  → 500 Internal Server Error → { "detail": "An unexpected error occurred." }
              (never a traceback — logged server-side with request_id)

GET /health
  → 200 OK → { "status": "ok", "modules": { "member1": "loaded", "member2": "loaded", "member3": "loaded" } }
```
...
startup — all three underlying modules already default to
...
`http://localhost:3000` or whatever Member 5 ends up using) — not `"*"`,
...
| 4 | Genuine voice + wrong speaker | E2E | Real modules — tests that Member 4 doesn't just parrot Member 1's GENUINE call; speaker mismatch should still surface in `reasons` |
...
isolation — pure functions, plain input dicts/dataclasses in, expected
...
to return canned `AdapterResult`s — this is how tests 5, 7, 8, 9, 10 above
...
Members 1, 2, and 3's own repos (reuse those files — don't record new
...
require a running server — good for a beginner on Windows, no separate
...
# 7. Run the real end-to-end tests (slow — loads all three real models)
...
#    test page (Swagger UI) — upload a file there directly, no curl needed.
```

**Common errors and what they mean:**
- `ModuleNotFoundError: No module named 'inference'` (or similar) →
  Members 1–3 aren't on the Python path yet — redo step 4.
- `ModuleNotFoundError: No module named 'speechbrain'` (or `whisper`,
  `transformers`) → that specific member's own `requirements.txt` hasn't
  been installed into this venv. Member 4's venv needs Members 1–3's
  dependencies too, since it imports their code directly (not over a
  network call) — install each member's `requirements.txt` into the same
  venv, in addition to Member 4's own.
- First real test/API call is very slow → expected, this is model loading
  ...
  already implement.
- `ffmpeg not found` errors from Member 2 or Member 3's converters →
  ...
  on Windows) — this is a Member 2/3 dependency, not something Member 4
...
### M4.0 — Repository audit
- **Goal:** Confirm the findings in Section B against the actual files, before writing anything.
- **Files:** none created/changed — read-only pass over Member 1/2/3 folders.
...
- **Common errors:** trusting this document over the code — don't; if they disagree, the code wins and Section B needs a correction note.
...
- **Files:** `adapters/result.py`, additions to `core/pipeline.py` (dataclass definitions can live there or in a `core/models.py` — either is fine, keep it consistent).
- **Why:** every other milestone imports these types; getting the shape right first avoids rework.
- **What to do:** implement exactly the dataclasses in Section F.1–F.3.
...
- **Common errors:** using `None` and `0`/`""` inconsistently as "missing" — always `None`.
- **Do NOT change:** the field names in Section F once other milestones start depending on them, without updating every consumer.

### M4.3 — Config centralization
- **Goal:** All thresholds/weights/margins from Sections G–J live in `config.py`.
- **Files:** `config.py`.
- **Why:** Rule 17/18 — no magic numbers scattered through the codebase.
...
- **Common errors:** none yet — this file has no logic to break.
- **Do NOT change:** Member 1's `DETECTION_THRESHOLD`/`MIN_RELIABLE_DURATION_SEC`/`UNCERTAINTY_MARGIN` or Member 2's `PROTOTYPE_THRESHOLD`/`UNCERTAINTY_MARGIN` — those stay in their own files; Member 4's `config.py` only holds Member-4-owned values (it may *read* the others by importing them, but does not redefine them).

### M4.4 — Normalization (unit-tested, no I/O)
- **Goal:** Implement Section G's three transform functions.
- **Files:** `core/normalization.py`, `tests/unit/test_normalization.py`.
- **Why:** this is pure, easily-testable logic — get it right in isolation before it's wired into anything real.
...
- **Common errors:** off-by-direction bugs (forgetting speaker similarity inverts while the other two don't) — the unit tests exist specifically to catch this.
...
### M4.5 — Reliability engine (unit-tested)
...
- **Common errors:** double-penalizing the same signal in two different sections of the formula (e.g., letting "insufficient duration" hit both the availability section and the quality section inconsistently) — keep each signal's contribution in exactly one place.
- **Do NOT change:** Member 1's diagnostic field names — reliability reads them, it doesn't define them.

### M4.6 — Risk engine (unit-tested)
...
- **Common errors:** dividing by the full weight sum instead of the *available* weight sum when a channel is missing — this silently deflates the score.
- **Do NOT change:** `config.RISK_WEIGHTS` casually — any change here is a real product decision, log why.

### M4.7 — Decision engine (unit-tested)
...
- **Verify success:** a risk score of exactly `HIGH_THRESHOLD` (65) returns `INCONCLUSIVE` due to the boundary margin, not `HIGH` — confirms the margin logic actually engages.
- **Common errors:** off-by-one on `<=` vs `<` at boundary checks — test the exact edges, not just interior values.
- **Do NOT change:** which check runs first (the ordering in Section J's `decide()` is deliberate — Member-1-missing and reliability-floor checks must happen before the boundary-margin check).

### M4.8 — Explainability (unit-tested)
...
- **Verify success:** no reason string appears when its triggering condition is false — write a test that asserts absence, not just presence.
- **Common errors:** appending a reason unconditionally "just in case" — every append must be inside an `if` tied to a real field.
- **Do NOT change:** Member 3's own `reasons[]` text when reusing it — pass it through verbatim.

### M4.9 — Integration adapters
...
- **Why:** this is where Member 4 first touches real external code — do it carefully, in its own milestone, separate from the pure-logic milestones above.
- **What to do:** each adapter calls the real module function inside a `try/except`, checks for Member 1's `"error"` key explicitly, validates numeric fields with `math.isfinite()`, and always returns an `AdapterResult` — never lets an exception escape.
- **Command:** `pytest tests/integration/test_adapters_mocked.py -v` (mocked — doesn't need real models yet) then a manual real call: `python -c "from adapters import member1_adapter; print(member1_adapter.run('sample_audio/some_file.wav'))"`.
...
- **Do NOT change:** anything in Member 1's files; only the two explicitly-scoped changes in Member 2/3 files from Section D — show the diff for review before considering this milestone done.

### M4.10 — Core pipeline + mocked integration tests
- **Goal:** Wire M4.4–M4.9 together into `core/pipeline.run_analysis()`, fully testable without FastAPI.
...
- **Common errors:** accidentally importing a real model at module load time inside `pipeline.py` (breaks the "mockable without loading models" property) — real module calls must only happen inside the adapters, which is what gets mocked.
- **Do NOT change:** the milestone order — do not start FastAPI before this passes.

### M4.11 — End-to-end pipeline with real modules
- **Goal:** Run the full pipeline against real Member 1–3 code and real sample audio, no mocks.
...
- **What to do:** implement Section O's test scenarios 1–4 and 6 as real E2E tests.
...
- **Common errors:** the various `ModuleNotFoundError`s and `ffmpeg not found` issues listed in Section P — resolve the environment, not the code, when these appear.
- **Do NOT change:** any threshold to "make the test pass" — if a real sample produces a surprising result, investigate whether the *test sample* or the *pipeline* is at fault before touching Section G/I/J constants.
...
- **Do NOT change:** any decision/risk/reliability logic inside `api/main.py` — if you find yourself writing an `if` about risk scores here, it belongs in `core/`, move it.
...
- **Common errors:** `TestClient` needing the app's lifespan events triggered explicitly depending on FastAPI/Starlette version — check the installed version's docs if startup-loaded models aren't available in tests.
...
### M4.14 — Final integration, docs, cleanup
- **Goal:** Everything from M4.1–M4.13 runs cleanly together; README is accurate; no dead code or stray print-debugging left in `core/`/`adapters/`/`api/`.
...
- **Common errors:** README drifting from what the code actually does — read it back against the real files one more time (echoing M4.0).
- **Do NOT change:** scope — this milestone is cleanup and documentation, not new features; anything Member 5/Member 6 will need later stays as a note for future work, not something built now.
