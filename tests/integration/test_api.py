...
@patch("api.main.run_pipeline")
def test_analyze_scam_context_genuine_voice(mock_pipeline):
    mock_pipeline.return_value = AnalysisResult(
        request_id="scam-test-123",
        decision="HIGH",
        risk_score=85.0,
        reliability_score=80.0,
        human_review_required=True,
        reasons=[
            "High genuine-voice probability (raw score 0.06).",
            "Account suspension threat detected.",
            "High contextual/social-engineering risk detected despite low deepfake risk. The voice may be genuine, but the message contains indicators associated with credential harvesting or account-takeover scams.",
        ],
        warnings=["Speaker verification skipped: no reference audio was provided."],
        risk_breakdown=NormalizedRisk(
            deepfake_risk=6.0,
        ...
        evidence={
            "member1": AdapterResult(status="ok", data={"raw_score": 0.06}),
            "member2": AdapterResult(status="skipped"),
            "member3": AdapterResult(status="ok", data={"context_score": 85.0}),
        },
    )

    fake_audio = ("scam_call.wav", b"RIFF....WAVEfmt ....scam....", "audio/wav")
    response = client.post("/analyze", files={"audio": fake_audio})
    ...
    assert data["risk_breakdown"]["deepfake_risk"] == 6.0
    assert data["risk_breakdown"]["speaker_mismatch_risk"] is None
    assert data["risk_breakdown"]["context_risk"] == 85.0
    assert any("High contextual/social-engineering risk detected" in r for r in data["reasons"])
